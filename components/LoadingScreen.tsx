...
const LoadingScreen = ({ children }: { children: React.ReactNode }) => {
    ...
    useEffect(() => {
        ...
        const onReady = () => {
            // Wait for fonts + a small buffer for animations to initialize
    ...
    return (
        <>
            {/* Loading overlay */}
            <div
                className={`fixed inset-0 z-9999 bg-(--black) flex items-center justify-center transition-opacity duration-500 ${ready ? "opacity-0 pointer-events-none" : "opacity-100"}`}
                aria-hidden={ready}
            >
                <svg
