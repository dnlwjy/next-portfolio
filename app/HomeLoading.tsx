...
interface HomeLoadingProps {
    styles?: string
    children: (props: { markReady: (index: number) => void }) => React.ReactNode
}

const TOTAL_VIDEOS = 5

const HomeLoading = ({ children }: HomeLoadingProps) => {
    const [readyMap, setReadyMap] = useState<boolean[]>(
        Array(TOTAL_VIDEOS).fill(false)
    )
    ...
    const allReady = readyMap.every(Boolean)
